...
class AccountInvoiceImport(models.TransientModel):
    ...
    invoice_id = fields.Many2one("account.invoice", string="Draft Supplier Invoice to Update")
    ...
    def _prepare_create_invoice_vals(self, parsed_inv, import_config=False):
        ...
        if not config:
            if not partner.invoice_import_ids:
                raise UserError(
                    _("Missing Invoice Import Configuration on partner '{}'.").format(
        ...
        if config["invoice_line_method"].startswith("1line"):
            ...
            for line in parsed_inv["lines"]:
                ...
                if not il_vals.get("account_id") and il_vals.get("product_id"):
                    ...
                    raise UserError(
                        _(
                            "Account missing on product '{}' or on it's related " "category '{}'."
        ...
        if float_compare(invoice.amount_total, parsed_inv["amount_total"], precision_rounding=prec):
            ...
            if not invoice.tax_line_ids:
                ...
                invoice.message_post(
                    _("The total tax amount has been forced to {} {} ").format(
    ...
    def update_invoice_lines(self, parsed_inv, invoice, seller):
        ...
        if compare_res["to_remove"]:
            ...
            chatter.append(
                _("{} invoice line(s) deleted: {}").format(
        ...
        if compare_res["to_add"]:
            ...
            chatter.append(
                _("{} new invoice line(s) created: {}").format(
    ...
    def update_invoice(self):
        ...
        invoice.message_post(
            _("This invoice has been updated automatically via the import " "of file {}").format(
            ...
            for attach in msg_dict["attachments"]:
                ...
                if not import_configs:
                    logger.warning(
                        ...
                        "for partner '{}' ID {}.".format(partner.display_name, partner.id)
                    )
                    continue
                elif len(import_configs) == 1:
                    import_config = import_configs.convert_to_import_config()
