    ...
    def parse_fe_cr_invoice(self, xml_root, document_type):
        ...
        if float_compare(total_line, counters["lines"], precision_digits=prec):
            logger.warning(
                ...
                "rounded sum policies.".format(total_line, counters["lines"])
            )
        ...
        if msg_dict.get("attachments"):
            ...
            for clave in invoices:
                ...
                if "invoice_attachment" in invoices[clave]:
                    ...
                    logger.info(
                        "Attachment {}: {}. Trying to import it as an invoice".format(
                            i, invoice_filename
                        )
                    )
                    ...
                    if (
                        ...
                        if existing_inv:
                            logger.warning(
                                "Mail import: this supplier invoice already exists in Odoo (ID {} number {} supplier number {})".format(
                                    existing_inv.id,
                                    existing_inv.number,
                        ...
                        if not import_configs:
                            logger.warning(
                                "Mail import: missing Invoice Import Configuration for partner '{}'.".format(
