...
class Landed_cost_inherit(models.Model):
    ...
    def get_valuation_lines(self):
        ...
        for move in self.mapped('picking_ids').mapped('move_lines'):
            ...
            if move.product_id.cost_method in  ['fifo','standard'] or move.product_id.valuation == 'manual_periodic':
                raise UserError(_("You cannot apply landed costs on the chosen transfer(s). Landed costs can only be applied for products with automated inventory valuation and AVERAGE costing method."))
            vals = {
