...
class CarRepairTeams(models.Model):
	...
	team_member_ids = fields.Many2many('res.users','res_user_rel','car_team_id','user_id',string="Team Member")
...
class CarDiagnosys(models.Model):      
	...
	def consume_car_parts(self):
		...
		if setting.consume_parts:
			...
			raise Warning(_("Please select the Consume Parts option in Inventory Settings to consume Car Parts"))
...
class AccountAnalyticLine(models.Model):      
	...
	def create(self, values):
		...
		timesheet_id = values.get('timesheet_id') or self.env['car.diagnosys'].search([['car_repair_id','=',car_repair_id]],limit=1).id
		car_workorder_id = values.get('car_workorder_id') or self.env['car.workorder'].search([['car_repair_id','=',car_repair_id]],limit=1).id
