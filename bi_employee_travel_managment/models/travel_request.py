...
class My_travel_request(models.Model):
    ...
    request_by = fields.Many2one('hr.employee',string="Requested By",readonly=True)
    ...
    def action_create_expence(self): 
        ...
        for line in self.expense_ids :
            id_lst.append(line.id)
        res = self.env['hr.expense.sheet'].create({'name': self.travel_purpose ,'employee_id' : self.employee_id.id,
